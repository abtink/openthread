...
The following formats are used to publish DNS/SRP Service entry (available when `OPENTHREAD_CONFIG_TMF_NETDATA_SERVICE_ENABLE` is enabled):

- `netdata publish dnssrp anycast <serial-num>` to publish "DNS/SRP Service Anycast Address" with a given serial number to be published in the Thread Network Data.
...
A `netdata publish dnssrp` command will remove and replace any previous "DNS/SRP Service" entry that was being published (from earlier `netdata publish dnssrp` commands).
...
The following formats are used to unpublish DNS/SRP Service entry (available when `OPENTHREAD_CONFIG_TMF_NETDATA_SERVICE_ENABLE` is enabled):
