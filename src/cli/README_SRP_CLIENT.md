...
Usage: `srp client service add <instancename> <servicename> <port> [priority] [weight]`

Add a service with a given instance name, service name, port number, priority, and weight values. The priority and weight are optional and if not provided zero will be used.
...
Usage: `srp client service remove <instancename>`

Remove a service with a give instance name.

```bash
> srp client service remove ins2
Done
```
